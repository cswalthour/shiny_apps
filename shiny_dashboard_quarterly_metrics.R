...
#read_data <- readRDS("Surgery/psc_quarterly_metrics_detail.Rds")

#read_data_2 <- read_data %>%
  #group_by(field) %>% nest() %>% ungroup()

ui <- fluidPage(
  titlePanel("Quarterly Metrics by Practice"),
  sidebarLayout(
    sidebarPanel(
      selectInput("practice_input", "Practice Area",
                  choices = unique(c("Podiatry", "Orthopaedic", "General", "Hand",
    ...
    mainPanel(strong("SUMMARY MONTHLY DATA"),
      dataTableOutput("summary"),
      br(), br(),
      strong("DETAIL MONTHLY DATA"),
...
server <- function(input, output) {
          ...
          temp_tbl <- x %>% tidyr::unnest() %>% group_by(month) %>% tidyr::nest() %>%
            ...
            mutate(data = pbapply::pblapply(data, function(x){
              ...
              metrics <- tibble(num_cases = nrow(temp_temp_tbl),
                ...
                mutate(cases_sample = ifelse(cases_sample <=0, 1, cases_sample))
  ...
  output$detail <- renderDataTable({DT::datatable(data() %>%
                filter(field == input$practice_input) %>%
                dplyr::select(-c(month:cases_sample)) %>%
                distinct() %>%
